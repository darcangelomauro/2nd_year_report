...
\end{align}
where $t$ denotes a fictitious time that will be referred to as Monte Carlo time. This specifies a transition $(\mathbf{q}(0), \mathbf{p}(0)) \rightarrow (\mathbf{q}(t), \mathbf{p}(t))$ from an initial configuration to a new one.\newline
...
\item accept the new configuration with probability $\text{min}[1, \exp (H'-H)]$.
\end{enumerate}
Note that Hamiltonian dynamics preserves the value of the energy of the system, therefore $H' = H$ and $\text{min}[1, \exp (H'-H)] = 1$ if step 2 is exact. However, numerical integration of Hamilton's equations is a non-trivial matter that introduces errors, therefore $H' \neq H$ in general. The standard choice of numerical integrator in HMC is the so-called \textit{leapfrog} integrator. For details see Appendix ZZZZ.
